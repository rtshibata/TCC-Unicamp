...
class DecisionTree:
	...
	#construtor
	def __init__(self,data_reader,class_values,feature_selection_function,KBest):
		self.data = data_reader
		self.clf = pd.DataFrame() #empty
		self.class_values = class_values
		self.param = pd.DataFrame() #empty
		self.kbest = KBest
		self.f_select = feature_selection_function
	...
	def begin_alg(self):
		...
		treetype =self.f_select+"_"+str(self.kbest) 
		treename = treetype + ".dot"
		tree.export_graphviz(self.clf, out_file=treename)

		bashCommand = "dot -Tps {} -o {}.ps".format(treename,treetype)
