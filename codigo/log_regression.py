...
class LogisticRegression:
	...
	def begin_alg(self):
		...
		i=0
		for c in coef:
			print("{},attribute ({}): {}".format(header_line[i],i,c))
			i +=1
		print("maximum weights")
		print([i for i, j in enumerate(coef) if j == max(coef)])
		second = sorted(coef.copy())[-2]
		'''
		print([i for i, j in enumerate(coef) if j ==second])
		third = sorted(coef.copy())[-3]
		print([i for i, j in enumerate(coef) if j ==third])
		forth = sorted(coef.copy())[-4]		
		print([i for i, j in enumerate(coef) if j ==forth])
		fifth = sorted(coef.copy())[-5]		
		print([i for i, j in enumerate(coef) if j ==fifth])
		sixth = sorted(coef.copy())[-6]		
		print([i for i, j in enumerate(coef) if j ==sixth])
		'''
	...
	def begin_test(self,data_test):
		...
		#print(type(X.iloc[2,-1]))

		#clf = linear_model.LogisticRegression()
		#clf.fit(X,y)
		l_classes = self.clf.predict(X)
		score = self.clf.score(X,y)
		...
		#print maskTP.value_counts().ix[1,1]
		n_correct_players = maskCorrect.value_counts().ix[1,1] if True in maskCorrect.index else 0
